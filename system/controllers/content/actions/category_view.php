...
class actionContentCategoryView extends cmsAction {
     ...
     * @param array $forms Массив форм слова: [один, два, много]
     * @return string Склоненное слово
     */
    private function declension($number, $forms) {
        $n = abs($number) % 100;
        $n1 = $n % 10;
        if ($n > 10 && $n < 20) {
    ...
    public function run() {
        ...
        // Определяем $is_hide_items до использования
        ...
        if (!$is_hide_items) {
            ...
            if (!empty($ctype['options']['show_items_counts']) && !empty($category['item_count'])) {
                $items_count = $category['item_count'];
            } else {
                // Применяем фильтры для подсчета
                if ($ctype['is_cats']) {
                    if ($slug !== 'index' && !empty($category['id'])) {
                ...
                $items_count = $this->model->getContentItemsCount($ctype['name']);
                $this->model->resetFilters();
            }

            // Определяем формы склонения и иконку в зависимости от типа контента
            $content_types = [
                'discounts' => [
                    'forms' => ['активная скидка', 'активные скидки', 'активных скидок'],
                    'icon' => 'discount-icon',
                    'empty_text' => 'Нет активных скидок'
                ],
                'promocodes' => [
                    'forms' => ['активный промокод', 'активных промокода', 'активных промокодов'],
                    'icon' => 'promocode-icon',
                    'empty_text' => 'Нет активных промокодов'
                ],
                'discussions' => [
                    'forms' => ['активное обсуждение', 'активных обсуждения', 'активных обсуждений'],
                    'icon' => 'discussion-icon',
                    'empty_text' => 'Нет активных обсуждений'
                ]
            ];

            // Выбираем настройки для текущего типа контента
            $ctype_key = $ctype['name'];
            if (isset($content_types[$ctype_key])) {
                $forms = $content_types[$ctype_key]['forms'];
                $icon = $content_types[$ctype_key]['icon'];
                $empty_text = $content_types[$ctype_key]['empty_text'];
            } else {
                // Значения по умолчанию, если тип контента не найден
                $forms = ['запись', 'записи', 'записей'];
                $icon = 'default-icon';
                $empty_text = 'Нет записей';
            }

            // Формируем предложение с учетом склонения и иконки
            $items_count_text = sprintf('<span class="%s"></span> %d %s', $icon, $items_count, $this->declension($items_count, $forms));
        } else {
            // Текст для пустого списка с учетом типа контента
            $ctype_key = $ctype['name'];
            $items_count_text = isset($content_types[$ctype_key])
                ? sprintf('<span class="%s"></span> %s', $content_types[$ctype_key]['icon'], $content_types[$ctype_key]['empty_text'])
                : '<span class="default-icon"></span> Нет записей';
        }

        // Категории включены?
        if ($ctype['is_cats']) {
            // Фильтр по категории для списка записей
            if ($slug !== 'index' && !empty($category['id'])) {
                $this->model->filterCategory($ctype['name'], $category, $ctype['is_cats_recursive'], !empty($ctype['options']['is_cats_multi']));
            } elseif (!$ctype['is_cats_recursive']) {
                $this->model->filterCategory($ctype['name'], ['id' => 1], false, !empty($ctype['options']['is_cats_multi']));
        ...
        return $this->cms_template->render($tpl_file, [
            ...
            'items_count_text' => $items_count_text, // Передаем готовое предложение с иконкой
            'user'            => $this->cms_user
        ], $this->request);
    }
