...
<?php foreach ($menu as $id => $item) { ?>
    ...
    <li <?php if ($css_classes) { ?>class="<?php html(implode(' ', $css_classes)); ?>"<?php } ?>>
        ...
        <?php } else { ?>
            <a <?php if (!empty($item['title'])) {?>title="<?php echo html($item['title']); ?>"<?php } ?> class="<?php html(implode(' ', $css_aclasses)); ?><?php if ($item['level'] == 1) {?> top-menu-sk<?php } ?>" href="<?php echo !empty($item['url']) ? html($item['url'], false) : '#'; ?>" data-style="sm" <?php echo html_attr_str($item['attributes']); ?>>
                ...
                <?php if (!empty($item['title']) && empty($item['options']['hide_title']) && ($item['level']>1)) { ?>
