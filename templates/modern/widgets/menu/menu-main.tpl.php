<?php if ($widget->options['menu_type'] == 'navbar') { ?>
    ...
    <nav class="<?php echo implode(' ', $nav_class); ?>">
        ...
        <?php } ?>
        <div class="collapse<?php if (!empty($widget->options['toggler_right_menu'])) { ?> ml-auto flex-grow-0<?php } ?> navbar-collapse nav-main<?php if (!$widget->options['navbar_expand']) { ?> show<?php } ?>" id="target-<?php echo $widget->options['menu']; ?>">
            <?php
